# Реализованный функционал — Kick Chat
...
- При первом запуске без аккаунтов показывается ссылка для авторизации; после редиректа на `http://localhost:8765/callback` токен сохраняется.
...
**Макет:**
- **Шапка:** логотип «Kick Chat», бейдж канала (slug), метка «Обновлено…», кнопка **«Отключить»** (завершение работы софта).
